...
public:
   ...
   */
  Pathfinder(Field* activeFields[8][8],
   ...
   */
  Road astar_search(Field* start_field,
                    Field* finish_field,
                    int max_distance);
private:
   ...
   */
  Field* tryGetFieldInDistance(Field* field_a,
                               sf::Vector2i offset);
  ...
  float calculate_score(const sf::Vector2i& cell_a,
  ...
  float calculate_score_with_diagonals(const sf::Vector2i& cell_a,
                                       const sf::Vector2i& cell_b);
};
