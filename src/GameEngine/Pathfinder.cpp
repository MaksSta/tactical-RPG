...
Pathfinder::Pathfinder(Field* activeFields[8][8],
...
Road Pathfinder::astar_search(Field* start_field, Field* finish_field, int max_distance)
{
  ...
  // zainicjowanie wszystkich danych
...
Field* Pathfinder::tryGetFieldInDistance(Field* field_a,
                                         sf::Vector2i offset)
{
...
void Pathfinder::connect(Field* p,
                         Field* q)
{
  auto pid = id[p];
  auto qid = id[q];
...
bool Pathfinder::areConnected(Field* p,
                              Field* q)
{
  if (id[p] == id[q])
    return true;
...
float Pathfinder::calculate_score(const sf::Vector2i& cell_a,
                                  const sf::Vector2i& cell_b)
{
  return abs(cell_b.x - cell_a.x) + abs(cell_b.y - cell_a.y);
}

float Pathfinder::calculate_score_with_diagonals(const sf::Vector2i& cell_a,
                                                 const sf::Vector2i& cell_b)
{
